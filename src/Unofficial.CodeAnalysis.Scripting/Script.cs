...
namespace Unofficial.CodeAnalysis.Scripting
{
    ...
    public abstract class Script
    {
        ...
        internal static readonly MetadataReferenceProperties HostAssemblyReferenceProperties =
            GetAssemblyReferenceProperties(ImmutableArray.Create("<host>"));
            ...
            MetadataReference languageRuntimeReferenceOpt = null)
        {
            ...
            try
            {
                if (Previous == null)
                {
                    [Obsolete("Just skip obsolete error", false)]
                    static MetadataReference CreateMetadataReferenceFromAssembly(Assembly assembly, MetadataReferenceProperties properties)
                        => MetadataReference.CreateFromAssembly(assembly, properties);

#pragma warning disable CS0618
                    var corLib = CreateMetadataReferenceFromAssembly(typeof(object).GetTypeInfo().Assembly, default);
#pragma warning restore CS0618
                    ...
                    if (GlobalsType != null)
                    {
                        ...
                        if (!globalsAssembly.IsDynamic && !string.IsNullOrEmpty(globalsAssembly.Location))
                        {
#pragma warning disable CS0618
                            references.Add(CreateMetadataReferenceFromAssembly(globalsAssembly, HostAssemblyReferenceProperties));
#pragma warning restore CS0618
                        }
                    }
        ...
        private static MetadataReferenceProperties GetAssemblyReferenceProperties(ImmutableArray<string> aliases)
        {
            var properties = MetadataReferenceProperties.Assembly;
            var ctors = typeof(MetadataReferenceProperties).GetConstructors(BindingFlags.NonPublic | BindingFlags.Instance)!;
            var instance = ctors.First().Invoke([properties.Kind, properties.Aliases, properties.EmbedInteropTypes, true]);

            return ((MetadataReferenceProperties)instance).WithAliases(aliases);
        }
    }
    ...
    public sealed class Script<T> : Script
    {
        ...
        public new Task<ScriptState<T>> RunFromAsync(ScriptState previousState, Func<Exception, bool> catchException = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            {
                throw new ArgumentException("Starting state was incompatible with script.", nameof(previousState));
            }
        ...
        private static void ValidateGlobals(object globals, Type globalsType)
        {
            if (globalsType != null)
            {
                if (globals == null)
                {
                    throw new ArgumentException("The script requires access to global variables but none were given", nameof(globals));
                }
                ...
                {
                    throw new ArgumentException(string.Format("The globals of type '{0}' is not assignable to '{1}'", runtimeType, globalsTypeInfo), nameof(globals));
                }
            }
            else if (globals != null)
            {
                throw new ArgumentException("Global variables passed to a script without a global type", nameof(globals));
            }
        }
    }
}
