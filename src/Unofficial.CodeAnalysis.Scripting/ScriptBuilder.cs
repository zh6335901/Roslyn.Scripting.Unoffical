...
namespace Unofficial.CodeAnalysis.Scripting
{
    ...
    internal sealed class ScriptBuilder
    {
        ...
        private static void ThrowIfAnyCompilationErrors(DiagnosticBag diagnostics, DiagnosticFormatter formatter)
        {
            ...
            var filtered = ImmutableArray.CreateRange(diagnostics.AsEnumerable().Where(d => d.Severity == DiagnosticSeverity.Error));
            if (filtered.IsEmpty)
            {
                return;
            }
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (var pdbStreamOpt = emitDebugInformation ? new MemoryStream() : null)
            {
                ...
                var getBoundReferenceManager = compilation.GetType().GetMethod("GetBoundReferenceManager", BindingFlags.Instance | BindingFlags.NonPublic);
                var referenceManager = getBoundReferenceManager.Invoke(compilation, null);
                var getReferencedAssemblies = referenceManager.GetType().GetMethod("GetReferencedAssemblies", BindingFlags.Instance | BindingFlags.NonPublic);
                var referencedAssemblies = getReferencedAssemblies.Invoke(referenceManager, null);

                PropertyInfo keyProperty = null;
                PropertyInfo valueProperty = null;
                PropertyInfo identityProperty = null;

                foreach (object referencedAssembly in (System.Collections.IEnumerable)referencedAssemblies)
                {
                    if (keyProperty == null)
                        keyProperty = referencedAssembly.GetType().GetProperty("Key");

                    var key = keyProperty.GetValue(referencedAssembly);
                    var path = (key as PortableExecutableReference)?.FilePath;
                    if (path != null)
                    {
                        ...
                        if (valueProperty == null)
                            valueProperty = referencedAssembly.GetType().GetProperty("Value");
                        var value = valueProperty.GetValue(referencedAssembly);

                        if (identityProperty == null)
                            identityProperty = value.GetType().GetProperty("Identity");
                        var identity = identityProperty.GetValue(value) as AssemblyIdentity;

                        _assemblyLoader.RegisterDependency(identity, path);
                    }
                }
