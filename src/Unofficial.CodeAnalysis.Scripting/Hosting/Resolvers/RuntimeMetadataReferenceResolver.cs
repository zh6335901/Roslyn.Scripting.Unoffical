...
namespace Unofficial.CodeAnalysis.Scripting.Hosting
{
    ...
    internal sealed class RuntimeMetadataReferenceResolver : MetadataReferenceResolver, IEquatable<RuntimeMetadataReferenceResolver>
    {
        ...
        internal RuntimeMetadataReferenceResolver(
            ...
            Func<string, MetadataReferenceProperties, PortableExecutableReference>? fileReferenceProvider = null)
            : this(new RelativePathResolver(searchPaths.IsDefault ? [] : searchPaths, baseDirectory),
                   packageResolver,
                   gacFileResolver,
                   GetTrustedPlatformAssemblies(platformAssemblyPaths.IsDefault ? [] : platformAssemblyPaths),
                   fileReferenceProvider)
        {
        }
        ...
        public override ImmutableArray<PortableExecutableReference> ResolveReference(string reference, string? baseFilePath, MetadataReferenceProperties properties)
        {
            if (NuGetPackageResolver.TryParsePackageReference(reference, out string packageName, out string packageVersion))
            {
                if (PackageResolver != null)
                {
                    ...
                    return ImmutableArray.CreateRange(paths, path => _fileReferenceProvider(path, properties));
                }
            }
        ...
        internal static ImmutableArray<string> GetTrustedPlatformAssemblyPaths()
            => ((AppContext.GetData("TRUSTED_PLATFORM_ASSEMBLIES") as string)?.Split(Path.PathSeparator))?.ToImmutableArray() ?? [];
        ...
        {
            return HashCode.Combine(PathResolver,
                   HashCode.Combine(PackageResolver,
                   HashCode.Combine(GacFileResolver,
