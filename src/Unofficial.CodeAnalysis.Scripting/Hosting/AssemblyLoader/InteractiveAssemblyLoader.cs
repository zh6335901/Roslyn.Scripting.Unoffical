...
namespace Unofficial.CodeAnalysis.Scripting.Hosting
{
    ...
    public sealed partial class InteractiveAssemblyLoader : IDisposable
    {
        ...
        public void RegisterDependency(AssemblyIdentity dependency, string path)
        {
            ...
            {
                throw new ArgumentException("Absolute path expected", nameof(path));
            }
                ...
                if (assemblyFilePath != null)
                {
                    ...
                    if (loadedAssemblyWithEqualNameAndVersionOpt.Assembly != null)
                    {
                        ...
                        throw new InteractiveAssemblyLoaderException(
                            string.Format(null, "Assembly '{0}, Version={1}' has already been loaded from '{2}'. A different assembly with the same name and version can't be loaded: '{3}'.",
                            identity.Name,
                            identity.Version,
                    ...
                    if (!conflictingLoadedAssemblyOpt.IsDefault)
                    {
                        // error: attempt to load an assembly with the same identity as already loaded assembly but different content
                        throw new InteractiveAssemblyLoaderException(
                            string.Format(null, "Assembly '{0}' has already been loaded from '{1}'. A different assembly with the same name can't be loaded unless it's signed: '{2}'.",
                            identity.Name,
