...
using Unofficial.CodeAnalysis.Scripting.Diagnostics;
...
namespace Unofficial.CodeAnalysis.CSharp.Scripting
{
    internal sealed class CSharpScriptCompiler : ScriptCompiler
    {
        ...
        {
            var withTopLevelBinderFlags = options.GetType().GetMethod("WithTopLevelBinderFlags", BindingFlags.Instance | BindingFlags.NonPublic);

            return (CSharpCompilationOptions)withTopLevelBinderFlags.Invoke(options, new object[] { 0x4000000u });
        }
    }
}
